...
export function createMockGoogleProvider(overrides = {}) {
  ...
  const googleProvider = {
    ...
    invoke: vi.fn().mockImplementation(async (messages, options = {}) => {
      ...
      const modelConfig =
        GOOGLE_MODELS[options.model] || GOOGLE_MODELS['gemini-1.5-flash'];
      ...
      if (
        ...
        modelConfig.modelName.includes('thinking')
      ) {
...
export function createMockGoogleResponse(
  ...
  return {
    ...
    usageMetadata: {
      ...
      totalTokenCount:
        (options.promptTokens || 10) + (options.outputTokens || 20),
    },
  };
}

// Mock streaming response generator
export function createMockGoogleStreamResponse(
  chunks = ['Hello', ' world', '!'],
) {
...
export function createMockGoogleError(
