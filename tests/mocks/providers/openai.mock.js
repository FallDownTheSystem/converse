...
export function createMockOpenAIProvider(overrides = {}) {
  ...
  const openAIProvider = {
    ...
    invoke: vi.fn().mockImplementation(async (messages, options = {}) => {
      ...
      const modelConfig =
        OPENAI_MODELS[options.model] || OPENAI_MODELS['gpt-4'];
      ...
      if (modelConfig.supportsThinking && options.reasoning_effort) {
        const thinkingTokens =
          options.reasoning_effort === 'high' ? 5000 : 2000;
...
export function createMockOpenAIStreamResponse(
  chunks = ['Hello', ' world', '!'],
  options = {},
) {
      ...
      completions: {
        create: vi.fn().mockImplementation(async (params) => {
          ...
          if (behavior.throwError) {
            throw new Error(
              JSON.stringify(createMockOpenAIError(behavior.errorType)),
            );
          }
          ...
          if (params.stream) {
            ...
            return {
              async *[Symbol.asyncIterator]() {
                for (const chunk of createMockOpenAIStreamResponse(
                  chunks,
                  params,
                )) {
                  yield chunk;
