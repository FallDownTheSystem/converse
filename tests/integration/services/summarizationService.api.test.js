...
describe('SummarizationService Real API Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (hasOpenAI && hasGoogle) {
        logger.info('Running SummarizationService API tests with OpenAI and Google');
      } else if (hasOpenAI) {
        logger.warn(`Skipping Google tests: ${getSkipMessage(['GOOGLE'])}`);
      } else if (hasGoogle) {
        logger.warn(`Skipping OpenAI tests: ${getSkipMessage(['OPENAI'])}`);
      } else {
        logger.warn(`Skipping all tests: ${getSkipMessage(['OPENAI', 'GOOGLE'])}`);
      }
    } catch (error) {
  ...
  describe('OpenAI GPT-5 Tests', () => {
    testWithApiKeys({
      ...
      requireAll: true
    })('should generate concise title with GPT-5', async () => {
      const service = new SummarizationService(providers, config);

      const prompt = 'I need to implement a user authentication system with JWT tokens, password hashing using bcrypt, and role-based access control for an Express.js application';
    ...
    testWithApiKeys({
      requiredProviders: ['OPENAI'],
      requireAll: true
    })('should generate streaming summary with GPT-5', async () => {
      const service = new SummarizationService(providers, config);
      ...
      const summary = await service.generateStreamingSummary(content, currentFocus, 'gpt-5');
    ...
    testWithApiKeys({
      requiredProviders: ['OPENAI'],
      requireAll: true
    })('should generate final summary with GPT-5', async () => {
      const service = new SummarizationService(providers, config);
  ...
  describe('Google Gemini-2.5-Flash Tests', () => {
    testWithApiKeys({
      ...
      requireAll: true
    })('should generate concise title with Gemini-2.5-Flash', async () => {
      const service = new SummarizationService(providers, config);

      const prompt = 'Create a React component for displaying user profiles with avatar upload, bio editing, and social media links integration';
    ...
    testWithApiKeys({
      requiredProviders: ['GOOGLE'],
      requireAll: true
    })('should generate streaming summary with Gemini-2.5-Flash', async () => {
      const service = new SummarizationService(providers, config);
      ...
      const summary = await service.generateStreamingSummary(content, currentFocus, 'gemini-2.5-flash');
    ...
    testWithApiKeys({
      requiredProviders: ['GOOGLE'],
      requireAll: true
    })('should generate final summary with Gemini-2.5-Flash', async () => {
      const service = new SummarizationService(providers, config);
      ...
      const summary = await service.generateFinalSummary(content, 'gemini-2.5-flash');
  ...
  describe('Performance and Optimization Tests', () => {
    testWithApiKeys({
    ...
    })('should use fastest settings for summarization', async () => {
      const service = new SummarizationService(providers, config);
      ...
      for (const testCase of testCases) {
        ...
        logger.info(`${testCase.provider} (${testCase.model}) response time: ${elapsed}ms`);
    ...
    testWithApiKeys({
      requiredProviders: ['OPENAI', 'GOOGLE'],
      requireAll: false
    })('should handle fallback when primary model unavailable', async () => {
      // Create a service with mocked provider availability
      const mockProviders = {
        openai: providers?.openai || { isAvailable: () => false, invoke: async () => null },
        google: providers?.google || { isAvailable: () => false, invoke: async () => null }
      };
  ...
  describe('Auto-selection Tests', () => {
    testWithApiKeys({
      requiredProviders: ['OPENAI', 'GOOGLE'],
      requireAll: false
    })('should auto-select fastest available model', async () => {
      const service = new SummarizationService(providers, config);
  ...
  describe('Content Quality Tests', () => {
    testWithApiKeys({
      requiredProviders: ['OPENAI'],
      requireAll: true
    })('should maintain context in streaming summaries', async () => {
      const service = new SummarizationService(providers, config);
      ...
      const currentFocus = 'Optimizing query performance for tenant-specific aggregations';
    ...
    testWithApiKeys({
      requiredProviders: ['GOOGLE'],
      requireAll: true
    })('should create actionable final summaries', async () => {
      const service = new SummarizationService(providers, config);
