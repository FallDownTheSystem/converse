...
import {
  ...
  withHTTPTestServer,
} from '../../utils/HTTPMCPServerManager.js';
...
describe('File Validation Integration Tests', () => {
  describe('Chat Tool File Validation', () => {
    it('should return error when files do not exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(result.content[0].text).toContain(
          'Files not found: nonexistent-file-1.txt, nonexistent-file-2.txt',
        );

        logger.info(
          '[file-validation-test] Chat tool correctly rejected missing files',
        );
      });
    });
    ...
    it('should return error when images do not exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain(
          'Images not found: missing-image-1.png, missing-image-2.jpg',
        );

        logger.info(
          '[file-validation-test] Chat tool correctly rejected missing images',
        );
      });
    });
    ...
    it('should return error when both files and images do not exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info(
          '[file-validation-test] Chat tool correctly rejected missing files and images',
        );
      });
    });
    ...
    it('should work normally when all files exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info(
          '[file-validation-test] Chat tool processed existing file successfully',
        );
      });
    });
    ...
    it('should handle mix of existing and non-existing files', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain(
          'Files not found: nonexistent.txt, missing.md',
        );

        logger.info(
          '[file-validation-test] Chat tool correctly rejected mix of existing and missing files',
        );
      });
    });
  });
  ...
  describe('Consensus Tool File Validation', () => {
    it('should return error when files do not exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain(
          'Files not found: nonexistent-1.txt, nonexistent-2.txt',
        );

        logger.info(
          '[file-validation-test] Consensus tool correctly rejected missing files',
        );
      });
    });
    ...
    it('should return error when images do not exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(result.isError).toBe(true);
        expect(result.content[0].text).toContain(
          'Images not found: missing-consensus-1.png, missing-consensus-2.jpg',
        );

        logger.info(
          '[file-validation-test] Consensus tool correctly rejected missing images',
        );
      });
    });
    ...
    it('should work normally when all files exist', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info(
          '[file-validation-test] Consensus tool processed existing file successfully',
        );
      });
    });
  });
  ...
  describe('Path Resolution', () => {
    it('should handle absolute paths correctly', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const absolutePath = getTestAbsolutePath(
          'Windows',
          'System32',
        ...
        logger.info(
          '[file-validation-test] Correctly handled absolute path validation',
        );
      });
    });
    ...
    it('should handle relative paths correctly', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info(
          '[file-validation-test] Correctly handled relative path validation',
        );
      });
    });
  });
