...
async function runTest() {
  try {
    ...
    console.log(
      `✓ Components loaded: ${Object.keys(tools).length} tools, ${Object.keys(providers).length} providers`,
    );
    ...
    try {
      ...
      if (chatResult.content?.[0]?.text) {
        ...
        console.log(
          `Response length: ${chatResult.content[0].text.length} characters`,
        );
    ...
    try {
      ...
      try {
        ...
        console.log(
          '✗ Error handling failed: Expected error but call succeeded',
        );
        results.push({
          test: 'Error Handling',
          success: false,
