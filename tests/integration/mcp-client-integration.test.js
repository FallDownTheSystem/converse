...
describe('MCP Client Integration Test Suite', () => {
  ...
  describe('Real API Integration via MCP Client', () => {
    it.skipIf(!hasAnyApiKey)('should execute real API calls through MCP protocol', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    it.skipIf(!hasAnyApiKey)('should handle consensus with real APIs via MCP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'consensus',
          arguments: {
