...
describe('OpenRouter API Integration Tests', () => {
  ...
  describe('Streaming Functionality', () => {
    it.skipIf(!hasOpenRouter())('should support basic streaming with kimi-k2', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[openrouter-streaming-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[openrouter-streaming-test] Kimi K2 streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasOpenRouter())('should support streaming with qwen3-coder', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test direct provider streaming with code model
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');

        const messages = [{ role: 'user', content: 'Write a simple function that adds 5 + 7. Just the code.' }];
        const streamResult = await openrouterProvider.invoke(messages, {
          config,
          model: 'qwen3-coder',
          stream: true,
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[openrouter-coder-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[openrouter-coder-test] Qwen3 Coder streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasOpenRouter())('should support streaming with qwen3-thinking model (reasoning events)', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test direct provider streaming with reasoning model
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');

        const messages = [{ role: 'user', content: 'What is 13 multiplied by 17? Show your calculation.' }];
        const streamResult = await openrouterProvider.invoke(messages, {
          config,
        ...
        for await (const event of streamResult) {
          ...
          if (event.type === 'thinking') {
            logger.info(`[openrouter-thinking-test] Thinking: ${event.content.substring(0, 100)}...`);
          } else {
            logger.info(`[openrouter-thinking-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
          }
        }
        ...
        if (thinkingEvents.length > 0) {
          logger.info(`[openrouter-thinking-test] Found ${thinkingEvents.length} thinking events`);
        }
        ...
        logger.info('[openrouter-thinking-test] Qwen3 Thinking streaming test completed successfully');
      });
    }, 90000);

    it.skipIf(!hasOpenRouter())('should support streaming with openrouter/auto model', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test direct provider streaming with auto-routing model
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');

        const messages = [{ role: 'user', content: 'What is 2 + 2? Reply with just the number.' }];
        const streamResult = await openrouterProvider.invoke(messages, {
          config,
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[openrouter-auto-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        if (endEvent.metadata.actual_provider) {
          logger.info(`[openrouter-auto-test] Auto-routing used provider: ${endEvent.metadata.actual_provider}`);
        }
        ...
        logger.info('[openrouter-auto-test] OpenRouter Auto streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasOpenRouter())('should handle streaming errors gracefully', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');
          ...
          try {
            for await (const event of streamResult) {
              ...
              if (event.type === 'error') {
                ...
                logger.info(`[openrouter-error-test] Error event: ${event.error.message}`);
                break;
              }
            }
          } catch (streamError) {
            // Error during streaming is also acceptable
            logger.info(`[openrouter-error-test] Streaming error caught: ${streamError.message}`);
          }
        ...
        logger.info('[openrouter-error-test] Error handling streaming test completed');
      });
    }, 30000);

    it.skipIf(!hasOpenRouter())('should include OpenRouter-specific metadata in streaming', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');
        ...
        if (endEvent.metadata.request_id) {
          ...
          logger.info(`[openrouter-metadata-test] Request ID: ${endEvent.metadata.request_id}`);
        }

        logger.info('[openrouter-metadata-test] OpenRouter metadata streaming test completed');
      });
    }, 60000);

    it.skipIf(!hasOpenRouter())('should properly handle usage reporting in streaming mode', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');
        ...
        expect(usageData.total_tokens).toBe(usageData.input_tokens + usageData.output_tokens);

        logger.info(`[openrouter-usage-test] Usage - Input: ${usageData.input_tokens}, Output: ${usageData.output_tokens}, Total: ${usageData.total_tokens}`);
        logger.info('[openrouter-usage-test] Usage reporting streaming test completed');
      });
    }, 60000);

    it.skipIf(!hasOpenRouter())('should handle multiple model streaming scenarios', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const { openrouterProvider } = await import('../../../../src/providers/openrouter.js');
        ...
        logger.info('[openrouter-multi-test] Multiple model streaming test completed');
      });
    }, 120000); // Longer timeout for multiple models
  });
