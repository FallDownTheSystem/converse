...
describe('OpenRouter Feature-Specific Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasOpenRouter()) {
        logger.warn('[openrouter-features-test] OpenRouter API key or referer not found - tests will be skipped');
      } else {
        logger.info('[openrouter-features-test] Running OpenRouter feature tests');
      }
    } catch (error) {
    ...
    it.skipIf(!hasOpenRouter())('should handle thinking models if available', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        if (!result.isError) {
          ...
          logger.info('[openrouter-features-test] Qwen Thinking test completed');
        } else {
          logger.info('[openrouter-features-test] Qwen Thinking not available, test skipped');
        }
      });
    }, 90000);
  });
  ...
  describe('Multi-Model Access', () => {
    it.skipIf(!hasOpenRouter())('should access diverse model ecosystem', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info('[openrouter-features-test] Multi-model access test completed');
      });
    }, 90000);
  });

  describe('Dynamic Model Support', () => {
    it.skipIf(!hasOpenRouter())('should support custom model paths', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info('[openrouter-features-test] Dynamic model support test completed');
      });
    }, 60000);
  });

  describe('Consensus with OpenRouter Models', () => {
    it.skipIf(!hasOpenRouter())('should participate in consensus gathering', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const models = [
          { model: 'k2' },
          { model: 'qwen-coder' }
