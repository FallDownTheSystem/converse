...
describe('Mistral Feature-Specific Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasMistral()) {
        logger.warn(
          '[mistral-features-test] Mistral API key not found - tests will be skipped',
        );
      } else {
  ...
  describe('Advanced Model Features', () => {
    it.skipIf(!hasMistral())(
      'should handle Magistral model if available',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          if (!result.isError) {
            ...
            logger.info(
              '[mistral-features-test] Magistral not available, test skipped',
            );
          }
        });
      },
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const result = await client.callTool({
            ...
            arguments: {
              prompt:
                'Is mathematics important for programming? Answer with "Yes" or "No" only.',
              models,
  ...
  describe('Language Capabilities', () => {
    it.skipIf(!hasMistral())(
      'should handle multilingual requests',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
              prompt:
                'Say "Hello" in French, Spanish, and German. Format: Language: greeting',
              model: 'mistral-medium',
              temperature: 0.3,
            },
          });
