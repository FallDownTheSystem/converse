...
describe('Mistral API Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasMistral()) {
        logger.warn(
          '[mistral-api-test] Mistral API key not found - tests will be skipped',
        );
      } else {
  ...
  describe('Basic Chat Functionality', () => {
    it.skipIf(!hasMistral())(
      'should work with Mistral Medium via HTTP',
      async () => {
    ...
    it.skipIf(!hasMistral())(
      'should work with Magistral Small for fast responses',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const firstResult = await client.callTool({
            ...
            arguments: {
              prompt:
                'Remember this fruit: apple. Just say "Remembered" to confirm.',
              model: 'mistral-medium',
              temperature: 0,
            },
          });
          ...
          logger.info(
            '[mistral-api-test] Conversation continuity test completed',
          );
        });
      },
      120000,
    ...
    it.skipIf(!hasMistral())(
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            `[mistral-api-test] Performance test completed in ${duration}ms`,
          );
        });
      },
      40000,
  ...
  describe('Streaming Functionality', () => {
    testWithApiKeys({
      ...
      'should support streaming with Magistral Medium',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          for await (const event of streamResult) {
            ...
            eventCount++;
            logger.info(
              `[mistral-streaming-test] Event: ${event.type}`,
          ...
          logger.info(
            '[mistral-streaming-test] Streaming test completed successfully',
          );
        });
      },
      60000,
    ...
    testWithApiKeys({
      ...
      'should support streaming with Magistral Small for fast responses',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const messages = [
            {
              role: 'user',
              content: 'What is 5 + 7? Answer briefly with just the number.',
            },
          ];
          const streamResult = await mistralProvider.invoke(messages, {
            config,
          ...
          for await (const event of streamResult) {
            ...
            if (event.type === 'delta' && event.content) {
              logger.info(
                `[mistral-small-streaming-test] Delta: "${event.content.substring(0, 100)}..."`,
              );
            }
          ...
          logger.info(
            '[mistral-small-streaming-test] Small model streaming test completed',
          );
        });
      },
      60000,
    ...
    testWithApiKeys({
      ...
      'should support streaming with Mistral Medium multimodal model',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const messages = [
            {
              role: 'user',
              content: 'Write a short 2-line poem about programming.',
            },
          ];
          const streamResult = await mistralProvider.invoke(messages, {
            config,
          ...
          logger.info(
            '[mistral-medium-streaming-test] Multimodal model streaming test completed',
          );
        });
      },
      90000,
