...
describe('Multi-Provider Error Handling Tests', () => {
  ...
  const hasAnyNewApiKey = () =>
    hasAnthropic() || hasDeepSeek() || hasMistral() || hasOpenRouter();
  ...
  beforeAll(async () => {
    try {
      ...
      if (availableProviders.length > 0) {
        logger.info(
          `[multi-provider-error-test] Running tests with providers: ${availableProviders.join(', ')}`,
        );
      } else {
        logger.warn(
          '[multi-provider-error-test] No new provider API keys found - tests will be skipped',
        );
      }
    } catch (error) {
  ...
  describe('Invalid Model Names Across Providers', () => {
    it.skipIf(!hasAnyNewApiKey())(
      'should handle invalid model names gracefully across all available providers',
      async () => {
          ...
          for (const testCase of testCases) {
            ...
            if (result.isError) {
              expect(result.content[0].text).toMatch(
                /(model|not found|not available|does not exist|404|provider error)/i,
              );
              logger.info(
                `[multi-provider-error-test] ${testCase.provider} handled invalid model correctly`,
              );
            } else {
              // If no error, it means a fallback was used
              expect(result.content).toBeDefined();
              logger.info(
                `[multi-provider-error-test] ${testCase.provider} used fallback for invalid model`,
              );
            }
          }
        });
  ...
  describe('Rate Limiting Across Providers', () => {
    it.skipIf(!hasAnyNewApiKey() || (!hasAnthropic() && !hasDeepSeek()))(
      'should handle rate limiting across multiple providers',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const providers = [];
          ...
          if (providers.length < 2) {
            logger.info(
              '[multi-provider-error-test] Not enough providers for rate limiting test',
            );
            return;
          }
          ...
          const totalSuccess = Object.values(successByProvider).reduce(
            (a, b) => a + b,
            0,
          );
          ...
          logger.info(
            '[multi-provider-error-test] Rate limiting test results:',
            successByProvider,
          );
        });
      },
