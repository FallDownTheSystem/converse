...
describe('Codex Provider E2E Tests', () => {
  ...
  describe('Basic Chat Functionality', () => {
    ...
    }, 90000); // 90s timeout (Codex can be slow)

    testWithApiKeys({
      requiredProviders: ['CODEX'],
      requireAll: true
    })('should maintain conversation continuity (thread resumption)', async () => {
      await withHTTPTestServer(async (client) => {
        ...
        logger.info(`[codex-api-test] First message completed, continuation_id: ${conversationId}`);
        ...
        logger.info('[codex-api-test] Conversation continuity test completed');
      });
    }, 180000); // 180s timeout (two Codex calls)
  });
  ...
  describe('Streaming Support', () => {
    testWithApiKeys({
      requiredProviders: ['CODEX'],
      requireAll: true
    })('should work with streaming responses', async () => {
  ...
  describe('Async Mode', () => {
    testWithApiKeys({
      requiredProviders: ['CODEX'],
      requireAll: true
    })('should work with async mode', async () => {
      await withHTTPTestServer(async (client) => {
        // Submit async job
        const result = await client.callTool({
          name: 'chat',
        ...
        const continuationMatch = responseText.match(/continuation_id:\s*([^\s,]+)/);
    ...
    }, 120000); // 120s timeout for async job
  });
  ...
  describe('Error Handling', () => {
    testWithApiKeys({
      requiredProviders: ['CODEX'],
      requireAll: true
    })('should handle invalid continuation IDs gracefully', async () => {
      await withHTTPTestServer(async (client) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
