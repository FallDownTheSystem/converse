...
describe('Provider Image Processing Integration Tests', () => {
  ...
  describe('XAI Grok-4 Image Processing', () => {
    testWithApiKeys({ requiredProviders: ['XAI'], requireAll: true })('should process image with absolute path', async () => {
    ...
    testWithApiKeys({ requiredProviders: ['XAI'], requireAll: true })('should process image with relative path', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ requiredProviders: ['XAI'], requireAll: true })('should handle multiple images', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Google Gemini Pro Image Processing', () => {
    testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should process image with absolute path', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should process image with relative path', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should handle multiple images', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Provider Comparison', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'], requireAll: true })('should get consistent results across providers', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const providers = [
