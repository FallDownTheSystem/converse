  ...
  beforeAll(async () => {
    try {
      ...
      if (availableProviders.length >= 2) {
        logger.info(`[multi-provider-test] Running tests with providers: ${availableProviders.join(', ')}`);
      } else {
        logger.warn('[multi-provider-test] Less than 2 providers available - most tests will be skipped');
      }
    } catch (error) {
  ...
  describe('Main Provider Consensus', () => {
    testWithApiKeys({
      ...
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info(`[multi-provider-test] Main provider consensus completed with ${models.length} providers`);
      });
    }, 120000);
  ...
  describe('Mixed Provider Consensus', () => {
    it.skipIf(!hasAnyMainProvider || (!hasAnthropic() && !hasDeepSeek()))('should combine main and new providers', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        if (models.length < 2) {
          logger.info('[multi-provider-test] Not enough providers for mixed consensus');
          return;
        }
        ...
        logger.info(`[multi-provider-test] Mixed provider consensus completed with ${models.length} providers`);
      });
    }, 150000);
  });
  ...
  describe('All Provider Stress Test', () => {
    it('should handle consensus with all available providers', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        if (models.length < 2) {
          logger.info('[multi-provider-test] Not enough providers for stress test');
          return;
        }
        ...
        logger.info(`[multi-provider-test] All-provider stress test completed with ${models.length} providers, ${successCount} successful`);
      });
  ...
  describe('Error Recovery in Consensus', () => {
    testWithApiKeys({
      ...
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(consensusResult.status).toBe('consensus_complete');
        expect(consensusResult.successful_initial_responses).toBeGreaterThan(0);
        expect(consensusResult.successful_initial_responses).toBeLessThan(models.length);

        logger.info('[multi-provider-test] Partial failure recovery test completed');
      });
    }, 120000);
  });
});
