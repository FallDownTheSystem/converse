...
describe('XAI Image Processing Tests', () => {
  ...
  describe('Grok-4 Image Processing', () => {
    testWithApiKeys({
      ...
      'should process image with absolute path',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            ...
            arguments: {
              prompt:
                'What is shown in this image? Please describe what you see.',
              images: [
                'C:\\Users\\Juugo\\Documents\\Projects\\converse\\test_image.png',
              ],
              model: 'grok-4',
              temperature: 0,
            },
    ...
    testWithApiKeys({
      ...
      'should process base64 encoded image',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const result = await client.callTool({
            ...
            arguments: {
              prompt:
                'What do you see in this image? Just give a brief description.',
              images: [base64Image],
              model: 'grok-4',
              temperature: 0,
            },
          ...
          if (result.isError) {
            console.error(
              '[xai-image-test] Base64 test error:',
              result.content[0].text,
            );
            console.error(
              '[xai-image-test] Full result:',
          ...
          expect(responseText).toMatch(
            /(fruit|apple|grape|food|produce|vegetable)/,
          );
        });
      },
      120000,
  ...
  describe('Image Processing with Text Conversations', () => {
    testWithApiKeys({
      ...
      'should maintain conversation context with images',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const firstResult = await client.callTool({
            ...
            arguments: {
              prompt:
                'What animal is in this image? Remember it for our next interaction.',
              images: [baboonBase64],
              model: 'grok-4',
              temperature: 0,
            },
          ...
          expect(secondResult.content[0].text.toLowerCase()).toMatch(
            /(baboon|monkey|primate|mandrill)/,
          );

          logger.info(
            '[xai-image-test] Conversation with image context completed',
          );
        });
      },
      120000,
