...
describe('XAI API Integration Tests', () => {
  ...
  describe('Streaming Functionality', () => {
    testWithApiKeys({
      requiredProviders: ['XAI'],
      requireAll: true
    })('should support streaming with Grok-4', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test direct provider streaming (bypasses HTTP MCP for now)
        const { xaiProvider } = await import('../../../../src/providers/xai.js');
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[xai-streaming-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[xai-streaming-test] Streaming test completed successfully');
      });
    }, 60000);
    ...
    testWithApiKeys({
      requiredProviders: ['XAI'],
      requireAll: true
    })('should support streaming with live search (Grok-4)', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test live search streaming
        const { xaiProvider } = await import('../../../../src/providers/xai.js');

        const messages = [{ role: 'user', content: 'What is the current stock price of Tesla (TSLA)?' }];
        const streamResult = await xaiProvider.invoke(messages, {
          config,
          model: 'grok-4-0709',
          stream: true,
        ...
        for await (const event of streamResult) {
          ...
          if (event.type === 'usage' && event.usage.search_sources_used) {
            logger.info(`[xai-search-test] Search sources used: ${event.usage.search_sources_used}, cost: $${event.usage.search_cost_estimate}`);
          }
        }
    ...
    testWithApiKeys({
      requiredProviders: ['XAI'],
      requireAll: true
    })('should support streaming with all Grok models', async () => {
    ...
    }, 180000); // 3 minutes for all models

    testWithApiKeys({
      requiredProviders: ['XAI'],
      requireAll: true
    })('should handle streaming errors gracefully', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        try {
          ...
          logger.info('[xai-error-test] Error handling streaming test completed');
        } catch (error) {
          // Direct error from invoke call is also acceptable
          expect(error.name).toBe('XAIProviderError');
          logger.info('[xai-error-test] Direct error handling test completed');
        }
      });
    }, 60000);
  });
