...
describe('XAI Feature-Specific Tests', () => {
  ...
  describe('Multi-Model Consensus with XAI', () => {
    testWithApiKeys({
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const models = [{ model: 'grok' }, { model: 'grok-4' }];
  ...
  describe('Multi-Provider Consensus', () => {
    testWithApiKeys({
      ...
      'should work in multi-provider consensus',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const models = [];
          ...
          if (models.length < 2) {
            logger.info(
              '[xai-features-test] Skipping multi-provider test - not enough providers',
            );
            return;
          }
          ...
          logger.info(
            `[xai-features-test] Multi-provider consensus test completed with ${models.length} providers`,
          );
        });
      },
      120000,
