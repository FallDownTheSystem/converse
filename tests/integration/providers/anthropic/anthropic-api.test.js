...
describe('Anthropic API Integration Tests', () => {
  ...
  describe('Streaming Support', () => {
    it.skipIf(!hasAnthropic())('should support streaming with Claude Haiku', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[anthropic-streaming-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[anthropic-streaming-test] Streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasAnthropic())('should support streaming with thinking models (Claude Sonnet 4)', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[anthropic-thinking-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[anthropic-thinking-test] Thinking model streaming test completed successfully');
      });
    }, 120000); // Longer timeout for thinking models

    it.skipIf(!hasAnthropic())('should support streaming with Claude 3.5 Sonnet (multimodal model)', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test multimodal model streaming (without images for simplicity)
        const { anthropicProvider } = await import('../../../../src/providers/anthropic.js');

        const messages = [{ role: 'user', content: 'Write a haiku about streaming data.' }];
        const streamResult = await anthropicProvider.invoke(messages, {
          config,
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[anthropic-multimodal-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[anthropic-multimodal-test] Multimodal streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasAnthropic())('should handle streaming errors gracefully', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        logger.info('[anthropic-error-test] Streaming error handling test completed');
      });
    }, 30000);
  });
