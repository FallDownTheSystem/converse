...
describe('Anthropic Error Handling and Edge Cases', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasAnthropic()) {
        logger.warn(
          '[anthropic-error-test] Anthropic API key not found - tests will be skipped',
        );
      } else {
        logger.info(
          '[anthropic-error-test] Running Anthropic error handling tests',
        );
      }
    } catch (error) {
    ...
    it.skipIf(!hasAnthropic())(
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            `[anthropic-error-test] Rate limiting test completed with ${successCount}/3 successful`,
          );
        });
      },
      90000,
  ...
  describe('Conversation Continuity Edge Cases', () => {
    it.skipIf(!hasAnthropic())(
      'should handle conversation continuity with multiple models',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const firstResult = await client.callTool({
            ...
            arguments: {
              prompt:
                'Remember this sequence: Alpha, Beta, Gamma. Just acknowledge.',
          ...
          logger.info(
            '[anthropic-error-test] Cross-model conversation continuity test completed',
          );
        });
      },
      90000,
