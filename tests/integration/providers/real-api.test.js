...
describe('Real API Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      if (!hasAnyMainProvider) {
        ...
        logger.warn(`[real-api-test] ${skipMessage}`);
      } else {
        ...
        logger.info(`[real-api-test] Running real API tests with: ${availableProviders.join(', ')}`);
      }
  ...
  describe('Chat Tool with Real APIs via HTTP', () => {
    ...
    })('should complete a simple chat request with available provider via HTTP', async () => {
    ...
    testWithApiKeys({ 
      ...
      requireAll: true
    })('should work with OpenAI provider specifically via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ 
      ...
      requireAll: true
    })('should work with XAI provider specifically via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ 
      ...
      requireAll: true
    })('should work with Google provider specifically via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Consensus Tool with Real APIs via HTTP', () => {
    testWithApiKeys({ 
      requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] 
    })('should gather consensus from available providers via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
    ...
    testWithApiKeys({ 
      ...
      requireAll: true
    })('should test cross-feedback with multiple providers via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
  ...
  describe('Conversation Continuity via HTTP', () => {
    testWithApiKeys({ 
      requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] 
    })('should maintain conversation history across requests via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // First message
  ...
  describe('Error Handling with Real APIs via HTTP', () => {
    testWithApiKeys({ 
      requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] 
    })('should handle invalid model names gracefully via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ 
      requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] 
    })('should handle very large prompts appropriately via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
  ...
  describe('Provider-Specific Features via HTTP', () => {
    testWithApiKeys({ 
      requiredProviders: ['GOOGLE'],
      requireAll: true
    })('should support Google thinking mode via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ 
      requiredProviders: ['OPENAI'],
      requireAll: true
    })('should support OpenAI reasoning effort for O3 via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Performance with Real APIs via HTTP', () => {
    testWithApiKeys({ 
      requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] 
    })('should complete simple requests within reasonable time via HTTP', async () => {
      await withHTTPTestServer(async (client, manager) => {
