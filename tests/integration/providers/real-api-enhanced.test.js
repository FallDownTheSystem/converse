...
describe('Enhanced Real API Integration Tests via HTTP Client', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasAnyMainProvider) {
        const skipMessage = getSkipMessage(['OPENAI', 'XAI', 'GOOGLE']);
        logger.warn(`[real-api-enhanced-test] ${skipMessage}`);
      } else {
        const available = [];
  ...
  describe('Provider-Specific API Tests', () => {
    describe('OpenAI Integration', () => {
      testWithApiKeys({ requiredProviders: ['OPENAI'], requireAll: true })('should handle GPT-4o-mini requests correctly via HTTP client', async () => {
      ...
      testWithApiKeys({ requiredProviders: ['OPENAI'], requireAll: true })('should handle O3-mini with reasoning effort via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
      ...
      testWithApiKeys({ requiredProviders: ['OPENAI'], requireAll: true })('should handle token usage reporting via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
    ...
    describe('XAI Integration', () => {
      testWithApiKeys({ requiredProviders: ['XAI'], requireAll: true })('should handle Grok-4 requests correctly via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
      ...
      testWithApiKeys({ requiredProviders: ['XAI'], requireAll: true })('should handle Grok model aliases via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
    ...
    describe('Google Integration', () => {
      testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should handle Gemini Flash requests correctly via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
      ...
      testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should handle thinking mode correctly via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
          const result = await client.callTool({
            name: 'chat',
            arguments: {
      ...
      testWithApiKeys({ requiredProviders: ['GOOGLE'], requireAll: true })('should handle Gemini model aliases via HTTP client', async () => {
        await withHTTPTestServer(async (client, manager) => {
  ...
  describe('Multi-Provider Consensus Tests', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'], requireAll: false })('should gather consensus from multiple providers via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const models = [];
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'], requireAll: false })('should handle cross-feedback consensus via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const models = [];
        if (hasOpenAI) models.push({ model: 'gpt-4o-mini' });
        if (hasGoogle) models.push({ model: 'flash' });
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'], requireAll: false })('should handle consensus with files via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const models = [];
        if (hasOpenAI) models.push({ model: 'gpt-4o-mini' });
        if (hasGoogle) models.push({ model: 'flash' });
  ...
  describe('Advanced Real-World Scenarios', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle complex conversation flow via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Start conversation
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle file context processing via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle image context processing via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
            prompt: 'Describe what you see in this image. Be specific about the subject and any notable details.',
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle parameter validation with real APIs via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
        // Test temperature bounds
        const tempTest = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Error Handling with Real APIs', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle rate limiting gracefully via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle context length limits via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle provider switching on failure via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
  ...
  describe('Performance with Real APIs', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should meet performance benchmarks via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
    ...
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should handle concurrent real API calls via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
  ...
  describe('Provider Compatibility', () => {
    testWithApiKeys({ requiredProviders: ['OPENAI', 'XAI', 'GOOGLE'] })('should produce consistent outputs across providers via HTTP client', async () => {
      await withHTTPTestServer(async (client, manager) => {
