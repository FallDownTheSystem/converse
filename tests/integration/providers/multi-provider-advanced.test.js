...
describe('Advanced Multi-Provider Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (availableProviders.length >= 2) {
        logger.info(
          `[multi-provider-advanced-test] Running tests with providers: ${availableProviders.join(', ')}`,
        );
      } else {
        logger.warn(
          '[multi-provider-advanced-test] Less than 2 providers available - most tests will be skipped',
        );
      }
    } catch (error) {
  ...
  describe('Consensus with File Context', () => {
    testWithApiKeys({
      ...
      'should handle consensus with files',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          if (models.length < 2) {
            logger.info(
              '[multi-provider-advanced-test] Not enough providers for consensus with files',
            );
            return;
          }
          ...
          const result = await client.callTool({
            ...
            arguments: {
              prompt:
                'What programming language is this file written in? Answer with just the language name.',
              models: testModels,
              files: [testFile],
          ...
          logger.info(
            `[multi-provider-advanced-test] Consensus with files successful with ${testModels.length} providers`,
          );
        });
      },
      120000,
  ...
  describe('Complex Conversation Flow', () => {
    testWithApiKeys({
      ...
      'should handle complex multi-turn conversation',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          // Start conversation
          const start = await client.callTool({
            ...
            arguments: {
              prompt:
                'I want to learn about machine learning. What should I start with?',
              model: 'auto',
              temperature: 0.3,
            },
          });
          ...
          expect(shift.content[0].text.toLowerCase()).toMatch(
            /deep learning|neural|network/,
          );

          logger.info(
            '[multi-provider-advanced-test] Complex conversation flow completed',
          );
        });
      },
      90000,
  ...
  describe('Provider Output Consistency', () => {
    testWithApiKeys({
      ...
      'should produce consistent outputs across providers',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const testPrompt =
          ...
          Object.entries(results).forEach(([provider, answer]) => {
            ...
            logger.info(
              `[multi-provider-advanced-test] ${provider} answered: ${answer}`,
            );
          });
          ...
          logger.info(
            `[multi-provider-advanced-test] Unique answers: ${uniqueAnswers.size}, Total providers: ${Object.keys(results).length}`,
          );
        });
      },
      90000,
  ...
  describe('Concurrent Provider Requests', () => {
    testWithApiKeys({
      ...
      'should handle concurrent requests across providers',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            `[multi-provider-advanced-test] ${concurrentRequests} concurrent requests successful`,
          );
        });
      },
      60000,
