...
describe('Google API Integration Tests', () => {
  ...
  describe('Basic Chat Functionality', () => {
    testWithApiKeys({
      ...
      'should work with Gemini Flash via HTTP',
      async () => {
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const firstResult = await client.callTool({
            ...
            arguments: {
              prompt:
                'Remember this color: purple. Just say "Remembered" to confirm.',
              model: 'flash',
              temperature: 0,
            },
          });
          ...
          logger.info(
            '[google-api-test] Conversation continuity test completed',
          );
        });
      },
      120000,
    ...
    testWithApiKeys({
      ...
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            `[google-api-test] Performance test completed in ${duration}ms`,
          );
        });
      },
      40000,
  ...
  describe('Streaming Functionality', () => {
    testWithApiKeys({
      ...
      'should support streaming with Gemini Flash',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const messages = [
            { role: 'user', content: 'Count to 5, one number per sentence.' },
          ];
          ...
          for await (const event of streamResult) {
            events.push(event);
            logger.info(
              `[google-streaming-test] Event: ${event.type}`,
          ...
          expect(
            completionEvent.metadata.usage.total_tokens,
          ...
          logger.info(
            '[google-streaming-test] Streaming test completed successfully',
          );
        });
      },
      60000,
    ...
    testWithApiKeys({
      ...
      'should support streaming with thinking mode (Gemini Pro)',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const messages = [
            {
              role: 'user',
              content:
                'Solve: What is 17 * 23? Show your reasoning step by step.',
            },
          ];
          const streamResult = await googleProvider.invoke(messages, {
            config,
          ...
          for await (const event of streamResult) {
            ...
            if (event.type === 'delta' && event.content) {
              logger.info(
                `[google-thinking-test] Delta: "${event.content.substring(0, 100)}..."`,
              );
            }
          }
          ...
          logger.info(
            '[google-thinking-test] Thinking mode streaming test completed',
          );
        });
      },
      90000,
    ...
    testWithApiKeys({
      ...
      'should support streaming with web search grounding',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            '[google-grounding-test] Web search grounding streaming test completed',
          );
        });
      },
      90000,
