...
describe('DeepSeek API Integration Tests', () => {
  ...
  describe('Basic Chat Functionality', () => {
    ...
    it.skipIf(!hasDeepSeek())('should work with DeepSeek Reasoner model', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
  ...
  describe('Streaming Functionality', () => {
    it.skipIf(!hasDeepSeek())('should support basic streaming with deepseek-chat', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        for await (const event of streamResult) {
          events.push(event);
          logger.info(`[deepseek-streaming-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
        }
        ...
        logger.info('[deepseek-streaming-test] Basic streaming test completed successfully');
      });
    }, 60000);

    it.skipIf(!hasDeepSeek())('should support streaming with deepseek-reasoner', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        for await (const event of streamResult) {
          ...
          if (event.type === 'thinking') {
            logger.info(`[deepseek-reasoning-test] Thinking: ${event.content.substring(0, 100)}...`);
          } else {
            logger.info(`[deepseek-reasoning-test] Event: ${event.type}`, event.content ? `"${event.content.substring(0, 50)}..."` : '');
          }
        }
        ...
        if (thinkingEvents.length > 0) {
          logger.info(`[deepseek-reasoning-test] Found ${thinkingEvents.length} thinking events`);
        }

        logger.info('[deepseek-reasoning-test] Reasoning model streaming test completed');
      });
    }, 90000);

    it.skipIf(!hasDeepSeek())('should handle streaming errors gracefully', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const { deepseekProvider } = await import('../../../../src/providers/deepseek.js');
        ...
        try {
          ...
          for await (const event of streamResult) {
            events.push(event);
            if (event.type === 'error') {
              logger.info(`[deepseek-error-test] Error event: ${event.error.message}`);
            }
          }
        } catch (error) {
          // Expected behavior - should throw error for invalid model
          expect(error.message).toMatch(/model.*not (found|exist)/i);
          logger.info('[deepseek-error-test] Error handling test completed as expected');
        }
      });
    }, 30000);
  });
