...
describe('DeepSeek Feature-Specific Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasDeepSeek()) {
        logger.warn('[deepseek-features-test] DeepSeek API key not found - tests will be skipped');
      } else {
  ...
  describe('Specialized Model Features', () => {
    it.skipIf(!hasDeepSeek())('should excel at code generation tasks', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const result = await client.callTool({
          name: 'chat',
          arguments: {
            prompt: 'Write a Python function to calculate factorial. Include type hints.',
        ...
        logger.info('[deepseek-features-test] Code generation test completed');
      });
    }, 60000);
  });

  describe('Multi-Model Consensus with DeepSeek', () => {
    it.skipIf(!hasDeepSeek())('should participate in consensus gathering', async () => {
      await withHTTPTestServer(async (client, manager) => {
        const models = [
          { model: 'deepseek-chat' },
          { model: 'deepseek-coder' }
        ...
        const result = await client.callTool({
          name: 'consensus',
          arguments: {
            prompt: 'Is Python a good language for beginners? Answer with "Yes" or "No" only.',
            models,
  ...
  describe('Cross-Provider Integration', () => {
    it.skipIf(!hasDeepSeek())('should handle mixed code and chat requests', async () => {
      await withHTTPTestServer(async (client, manager) => {
