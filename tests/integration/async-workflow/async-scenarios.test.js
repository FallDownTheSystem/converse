...
describe('Async Scenarios Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasAnyApiKey) {
        logger.warn(
          '[async-scenarios] No API keys found - some tests will be skipped',
        );
      }
...
module.exports = { fibonacci };`,
  ...
  describe('Multi-step Async Conversations', () => {
    ...
    testWithAnyKey(
      'should handle async conversation with continuations',
      async () => {
        ...
        try {
          await withHTTPTestServer(async (client, manager) => {
            console.log(
              '[DEBUG TEST START] should handle async conversation with continuations at',
            ...
            console.log(
              '[DEBUG] Submitting step 1 async chat request at',
            ...
            try {
              ...
              console.log(
                '[DEBUG] Job ID for status checking:',
                continuationId1,
              );
              console.log(
                '[DEBUG] Conversation continuation ID:',
                step1Content.continuation_id,
              );
            } catch (parseError) {
              console.error(
                '[DEBUG] Failed to parse step 1 response:',
                parseError,
              );
            ...
            console.log(
              '[DEBUG] Starting to poll for job completion at',
              Date.now() - testStartTime,
              'ms...',
            );
            console.log(
              `[DEBUG] Looking for continuationId1: ${continuationId1}`,
            );
            ...
            while (!job1Complete && attempts < 30) {
              ...
              console.log(
                `[DEBUG] Poll attempt ${attempts + 1}/30 at`,
                Date.now() - testStartTime,
                'ms...',
              );
              ...
              console.log(
                `[DEBUG] Calling check_status for continuationId1: ${continuationId1} at`,
                Date.now() - testStartTime,
                'ms',
              );
              ...
              console.log(
                '[DEBUG] Status call completed in',
                statusCallTime,
                'ms',
              );
              ...
              try {
                ...
                console.error(
                  '[DEBUG] Failed to parse status response:',
                  parseError,
                );
              ...
              if (status.status === 'completed') {
                job1Complete = true;
                console.log(
                  '[DEBUG] Job 1 completed at',
                  Date.now() - testStartTime,
                  'ms! Will use original continuation ID for step 2.',
                );
                ...
                console.log(
                  `[DEBUG] Job still ${status.status}, continuing to poll...`,
                );
              }
              attempts++;
              ...
              console.log(
                `[DEBUG] Poll attempt ${attempts} took ${pollAttemptTime}ms`,
              );
            }

            expect(job1Complete).toBe(true);
            console.log(
              '[DEBUG] Using continuation ID for step 2:',
              continuationId1,
            );
            console.log(
              '[DEBUG] *** STARTING STEP 2 at',
              Date.now() - testStartTime,
              'ms ***',
            );
            ...
            console.log(
              '[DEBUG] Calling chat tool for step 2 at',
              Date.now() - testStartTime,
              'ms...',
            );
            ...
            console.log(
              '[DEBUG] Step 2 tool call completed in',
              step2SubmitTime,
              'ms at',
              Date.now() - testStartTime,
              'ms',
            ...
            try {
              ...
              console.log(
                '[DEBUG] Job ID 2 for status checking:',
                continuationId2,
              );
            } catch (parseError) {
              console.error(
                '[DEBUG] Failed to parse step 2 response:',
                parseError,
              );
            ...
            console.log(
              '[DEBUG] Starting to poll for step 2 job completion at',
              Date.now() - testStartTime,
              'ms...',
            );
            ...
            while (!job2Complete && attempts < 30) {
              ...
              console.log(
                `[DEBUG] Step 2 poll attempt ${attempts + 1}/30 at`,
                Date.now() - testStartTime,
                'ms...',
              );
              ...
              console.log(
                `[DEBUG] Calling check_status for step 2 continuationId2: ${continuationId2} at`,
                Date.now() - testStartTime,
                'ms',
              );
              ...
              console.log(
                '[DEBUG] Step 2 status call completed in',
                step2StatusTime,
                'ms',
              );
              ...
              if (status.status === 'completed') {
                ...
                console.log(
                  `[DEBUG] Step 2 job still ${status.status}, continuing to poll...`,
                );
              }
              ...
              console.log(
                `[DEBUG] Step 2 poll attempt ${attempts} took ${step2PollAttemptTime}ms`,
              );
            }

            if (!job2Complete) {
              console.error(
                '[DEBUG] Step 2 did not complete within 30 attempts',
              );
              console.error('[DEBUG] Current step:', currentStep);
              console.error(
                '[DEBUG] Total test time:',
                Date.now() - testStartTime,
                'ms',
              );
              throw new Error(
                `Step 2 did not complete. Current step: ${currentStep}`,
              );
            }
            ...
            console.log(
              '[DEBUG] *** TEST COMPLETED SUCCESSFULLY at',
              Date.now() - testStartTime,
              'ms ***',
            );
  ...
  describe('File Processing in Async Mode', () => {
    ...
    testWithOpenAI(
      'should handle file content in async requests',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          logger.info(
            '[async-scenarios] File processing in async mode verified',
          );
        });
      },
      80000,
  ...
  describe('Mixed Sync and Async Operations', () => {
    ...
    testWithAnyKey(
      'should handle mixed sync/async operations in same session',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          // 1. Sync operation
          ...
          const asyncResultContent =
            asyncFinalResult.content || asyncFinalResult.message || '';
          ...
          logger.info(
            '[async-scenarios] Mixed sync/async operations completed',
          );
        });
      },
      60000,
    ...
    it('should handle provider failures gracefully in async mode', async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        expect(errorMessage.toLowerCase()).toMatch(
          /invalid|model|not found|unknown/,
        );
  ...
  describe('Performance Under Load', () => {
    ...
    testWithAnyKey(
      'should maintain performance with rapid async submissions',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          const jobCount = 15;
          ...
          logger.info(
            `[async-scenarios] Submitted ${jobCount} jobs in ${submitTime}ms`,
          );
          ...
          const pollPromises = continuationIds.map(
            async (continuationId, index) => {
              let completed = false;
              let attempts = 0;
              ...
              while (!completed && attempts < maxAttempts) {
                ...
                try {
                  ...
                  if (status.status === 'completed') {
                    ...
                    const resultContent =
                      status.result?.content || status.result?.message || '';
                    expect(resultContent).toContain(expectedAnswer.toString());
                  }
                } catch (error) {
                  logger.error(
                    `[async-scenarios] Error polling job ${index}:`,
                    error,
                  );
                }
  ...
  describe('Real-world Async Consensus Scenarios', () => {
    ...
    testWithAnyKey(
      'should handle complex consensus with cross-feedback',
      async () => {
        await withHTTPTestServer(async (client, manager) => {
          ...
          const consensusContent = {
            continuation_id: consensusResult.continuation.id,
          };
          // Use continuation_id for status checking
          ...
          if (finalResult) {
            ...
            const resultContent =
              finalResult.content || finalResult.message || '';
            ...
            expect(resultContent.toLowerCase()).toMatch(
            ...
            console.log(
              '[DEBUG] Consensus completed but finalResult is null - this is acceptable for human-readable format',
            );
          }
          ...
          logger.info(
            '[async-scenarios] Complex consensus completed successfully',
          );
        });
      },
      90000,
