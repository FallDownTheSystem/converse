...
describe('Async Chat with Summarization Enabled', () => {
  ...
  beforeAll(async () => {
    ...
    logger.info(
      `Summarization enabled with model: ${config.summarization.model}`,
    );
  });
  ...
  testWithApiKeys({
    ...
    'should generate title and summaries during async chat',
    async () => {
      await withHTTPTestServer(async (client, manager) => {
        ...
        while (!completed && Date.now() - startTime < maxWaitTime) {
          ...
          if (status.accumulated_content && !accumulatedContentSeen) {
            ...
            logger.info(
              `✅ Accumulated content: ${status.accumulated_content.length} chars`,
            );
  ...
  testWithApiKeys({
    ...
    'should use minimal reasoning for fast summarization',
    async () => {
      await withHTTPTestServer(async (client, manager) => {
        const startTime = Date.now();
  ...
  testWithApiKeys({
    ...
    'should work with Gemini models',
    async () => {
      await withHTTPTestServer(async (client, manager) => {
        const asyncResult = await client.callTool({
          name: 'chat',
