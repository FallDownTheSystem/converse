...
describe('Cache System Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      ...
      if (!hasAnyApiKey) {
        logger.warn(
          '[cache-integration] No API keys found - some tests will be skipped',
        );
      }
  ...
  afterAll(async () => {
    ...
    if (cacheDir) {
      try {
        ...
        logger.warn(
          '[cache-integration] Failed to clean cache directory:',
          error,
        );
      }
    }
  ...
  describe('Memory to Disk Cache Transition', () => {
    ...
    testWithAnyKey(
      'should transition completed jobs from memory to disk cache',
      async () => {
    ...
    it.skip('should handle cache recovery after memory expires', async () => {
      ...
      try {
        ...
        if (!completed && lastError) {
          throw new Error(
            `Job never completed. Last error: ${JSON.stringify(lastError)}`,
          );
        }
        expect(completed).toBe(true);
      ...
      try {
        ...
        const recoveredContent =
          recoveredJsonStart >= 0
            ? JSON.parse(recoveredText.substring(recoveredJsonStart))
        ...
        expect(recoveredContent.result.content.toLowerCase()).toContain(
          'cached',
        );

        logger.info(
          '[cache-integration] Cache recovery after restart verified',
        );
      } finally {
      ...
      await withHTTPTestServer(
        async (client, manager) => {
          ...
          if (!completed) {
            ...
            throw new Error(
              `Large result job never completed after ${attempts} attempts. ${errorMsg}`,
            );
          }
          expect(completed).toBe(true);
          ...
          const lines = finalResult.result.content
            .split('\n')
            .filter((l) => l.trim());
      ...
      await withHTTPTestServer(
        async (client, manager) => {
          ...
          if (afterMemoryExpiry.isError || afterMemoryExpiry.error) {
            ...
            throw new Error(
              'Job expired from memory but should still be in disk cache',
            );
          }
          ...
          const stillAvailable =
            stillJsonStart >= 0
              ? JSON.parse(stillText.substring(stillJsonStart))
              : JSON.parse(stillText);
          ...
          if (expiredText.toLowerCase().includes('not found')) {
            ...
            const expired =
              expiredJsonStart >= 0
                ? JSON.parse(expiredText.substring(expiredJsonStart))
                : JSON.parse(expiredText);
            // Check either error field or the response indicates job not found
            expect(expired.error || expiredText.toLowerCase()).toContain(
              'not found',
            );
          }
    ...
    it.skip('should maintain performance with many cached jobs', async () => {
      ...
      await withHTTPTestServer(
        async (client, manager) => {
          ...
          const jobPromises = Array(jobCount)
            .fill(null)
            .map((_, i) =>
              client.callTool({
                name: 'chat',
          ...
          if (completedCount === 0) {
            console.warn(
              'WARNING: No jobs completed after 15 seconds, but some are still running',
            );
            // Don't fail the test if jobs are at least running
            const runningCount = retrievals.filter(
              (r) => !r.isError && !r.error,
            ).length;
          ...
          logger.info(
            `[cache-integration] Performance test: ${jobCount} jobs submitted in ${submitTime}ms, retrieved in ${retrieveTime}ms`,
          );
        },
        {
          env: {
