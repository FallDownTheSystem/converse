...
describe('Core Continuation Verification Tests', () => {
  ...
  test('should handle multiple chat continuations with math progression', async () => {
    ...
    const mathSteps = [
      { question: 'What is 2 + 2? Just give me the number.', expected: /4/, step: 1 },
      { question: 'Now what is 4 + 4? Just the number please.', expected: /8/, step: 2 },
      { question: 'What is 8 + 8? Just the number.', expected: /16/, step: 3 },
      { question: 'What is 16 + 16? Just the number.', expected: /32/, step: 4 },
      { question: 'What is 32 + 32? Just the number.', expected: /64/, step: 5 },
      { question: 'What is 64 + 64? Just the number.', expected: /128/, step: 6 },
      { question: 'What is 128 + 128? Just the number.', expected: /256/, step: 7 },
      { question: 'Finally, what is 256 + 256? Just the number.', expected: /512/, step: 8 }
    ];
  ...
  test('should handle consensus with math continuation', async () => {
    try {
      ...
      const result1 = await client.consensus('What is 2 + 2? Just give the number.', [
        { model: 'gpt-4o-mini' }
      ]);

      if (result1.isError) {
        console.log('Consensus 1 failed with error, skipping test:', result1.error);
        return; // Skip if no API keys
      }
      ...
      const result2 = await client.consensus('Now what is 4 + 4? Just the number.', [
        { model: 'gpt-4o-mini' }
      ...
      if (!result2.isError) {
        ...
        console.log('Consensus 2 successful, got answer:', response2Text.substring(0, 100));
      }
  ...
  test('should handle mixed chat and consensus with math progression', async () => {
    try {
      ...
      if (chatResult.isError) {
        console.log('Mixed test failed with error, skipping:', chatResult.error);
        return;
      }
      ...
      const consensusResult = await client.consensus('Now what is 4 + 4? Just the number please.', [
        { model: 'gpt-4o-mini' }
      ], { continuation_id: continuationId });
      ...
      if (!consensusResult.isError) {
        ...
        console.log('Mixed conversation successful, got answer:', consensusText.substring(0, 100));
      }
