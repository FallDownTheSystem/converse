...
export async function chatTool(args, dependencies) {
  try {
    ...
    if (async) {
      ...
      try {
        // Submit background job (job ID will be auto-generated by jobRunner)
        ...
        // IMPORTANT: job_id is for checking status, continuation_id is for conversation context
        return createToolResponse({
          content: JSON.stringify({
            message: 'Chat request submitted for background processing',
            job_id: jobId,  // The job ID to use with check_status
            continuation_id: conversationContinuationId,  // The conversation ID (will be returned when job completes)
            status: 'processing',
            async_execution: true
          }),
          continuation: {
            job_id: jobId,  // Job tracking ID
            continuation_id: conversationContinuationId,  // Conversation continuation ID
            status: 'processing'
          },
          async_execution: true
        });
